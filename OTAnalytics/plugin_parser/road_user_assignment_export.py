...
class RoadUserAssignmentCsvExporter(RoadUserAssignmentExporter):
    ...
    def _serialize(self, dtos: list[dict], export_mode: ExportMode) -> None:
        ...
        df = DataFrame(dtos, columns=ROAD_USER_ASSIGNMENT_DICT_KEYS)
        if all(df[col].isna().all() for col in _GEO_COLUMNS):
            df = df.drop(columns=_GEO_COLUMNS)
        df.to_csv(self._outputfile, index=False, header=not append, mode=write_mode)
