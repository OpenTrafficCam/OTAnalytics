...
def mark_last_detection_finished(track: Track) -> PythonTrack:
    last_detection = track.last_detection
    finished_last_detection = PythonDetection(
    ...
    detections = list(track.detections[:-1]) + [finished_last_detection]
    return PythonTrack(
        _id=track.id,
