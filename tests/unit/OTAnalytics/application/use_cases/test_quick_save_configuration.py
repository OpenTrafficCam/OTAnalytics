...
class TestQuickSaveConfiguration:
    ...
    async def test_save_otconfig(self) -> None:
        ...
        state.last_saved_config.get.assert_called_once()
        save_otconfig.assert_awaited_once_with(config_file.file)
        save_otflow.save.assert_not_called()

    async def test_save_not_supported_format(self) -> None:
    ...
    async def test_save_no_configuration_file(self) -> None:
        save_otflow = Mock()
        save_otconfig = AsyncMock()
